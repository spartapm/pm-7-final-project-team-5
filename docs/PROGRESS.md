...
현재 작업 기준 페이지는 **인플롯 0.2**입니다. `패턴노트 0.1` / `인사이트` 페이지는 deprecated.
...
| 1 | 로그인 및 회원가입 `node-id=0-116` | `/login`, `/signup/nickname`, `/welcome`, `/legal/terms`, `/legal/privacy`, `/auth/kakao/callback` | ✅ 필수 약관 → 카카오 REST(회원번호만, 이메일 없음) / 데모 → 닉네임 2~10자 → 환영 |
...
- Supabase: `accounts` (발행 baseline 포함), `plans` (`side`·`buy_min`·`buy_max`), `trades` (`traded_time`·계획 스냅샷), `insight_cards` (insert-only)
